...
def test_upsert_user_creates_and_returns():
    ...
    user = upsert_user("google_sub_1", "Alice", "alice@example.com", "https://avatar.url/a")
...
def test_upsert_user_updates_on_conflict():
    from app.auth import upsert_user
    upsert_user("sub2", "Bob", "bob@example.com", "")
    user2 = upsert_user("sub2", "Bobby", "bob@example.com", "https://new.avatar")
    assert user2["name"] == "Bobby"


def test_create_and_decode_jwt():
    from app.auth import create_jwt, decode_jwt, upsert_user
    user = upsert_user("sub3", "Carol", "carol@example.com", "")
...
def test_favorites_add_list_remove():
    ...
    user = upsert_user("sub4", "Dave", "dave@example.com", "")
    uid = user["id"]
...
def test_add_favorite_idempotent():
    ...
    user = upsert_user("sub5", "Eve", "eve@example.com", "")
    uid = user["id"]
...
def test_history_add_and_list():
    ...
    user = upsert_user("sub6", "Frank", "frank@example.com", "")
    uid = user["id"]

...
class TestPrivateDMUsesPlayerContext:
    ...
    async def test_system_prompt_includes_own_private_clue(self, mock_llm, collab_session, collab_puzzle):
        """Player A's private clue content appears in the system prompt for Player A."""
        mock_llm.set_response({"judgment": "无关", "response": "继续思考吧", "truth_progress": 0.1, "should_hint": False, "audience": "private"})
        ...
        assert "南海贸易" in system_prompt or "银行账户" in system_prompt, "Player A's private clue (bank record) not found in system prompt"

    async def test_system_prompt_excludes_other_player_clue(self, mock_llm, collab_session, collab_puzzle):
        ...
        assert "字迹潦草" not in system_prompt, "Player B's private clue content leaked into Player A's private prompt"
        assert "钱算什么" not in system_prompt, "Player B's private clue content leaked into Player A's private prompt"

    async def test_system_prompt_has_private_audience_flag(self, mock_llm, collab_session):
        """Private chat prompt must include the private audience instruction."""
        mock_llm.set_response({"judgment": "无关", "response": "好的", "truth_progress": 0.0, "should_hint": False, "audience": "private"})
        await dm_turn_private(collab_session, "uid-B", "日记里说了什么意思？")
        system_prompt = mock_llm.last_system_prompt

        assert "私密对话" in system_prompt, "Private audience instruction missing from private chat system prompt"
        assert "private" in system_prompt.lower(), "audience=private marker missing from private chat system prompt"

    async def test_player_b_private_clue_in_prompt_for_player_b(self, mock_llm, collab_session):
        ...
        assert "日记" in system_prompt, "Player B's private clue (diary) not found in their own private prompt"

    async def test_all_player_summary_in_private_prompt(self, mock_llm, collab_session):
        ...
        assert "player_2" in system_prompt or "player_3" in system_prompt, "All-player summary missing from private chat system prompt"
...
class TestPrivateChatHistoryIsolation:
    ...
    async def test_history_unchanged_after_private_turn(self, mock_llm, collab_session):
        ...
        assert len(collab_session.history) == history_before, "dm_turn_private must not append to session.history"

    async def test_shared_history_not_contaminated_across_private_turns(self, mock_llm, collab_session):
        ...
        assert len(collab_session.history) == 0, "Private chat exchanges must not pollute session.history"

    async def test_private_chat_uses_existing_history_as_read_only_context(self, mock_llm, collab_session):
        ...
        assert any(m["content"] == "公开问题" for m in last_messages), "dm_turn_private should pass existing history as context to LLM"
        assert any(m["content"] == "私密问题" for m in last_messages), "dm_turn_private should include the private message in LLM messages"

        # But history itself must be unchanged
        assert len(collab_session.history) == 2, "dm_turn_private must not extend session.history"
    ...
    async def test_private_turn_returns_string(self, mock_llm, collab_session):
        ...
        mock_llm.set_response({"judgment": "是", "response": "你的线索很关键！", "truth_progress": 0.5, "should_hint": False, "audience": "private"})
...
class TestPublicDMDoesNotLeakPrivate:
    ...
    async def test_public_prompt_does_not_include_other_player_clue_content(self, mock_llm, collab_session):
        """Player A's public question builds a prompt that excludes Player B's content."""
        mock_llm.set_response({"judgment": "无关", "response": "继续思考", "truth_progress": 0.0, "should_hint": False, "audience": "public"})
        ...
        assert "字迹潦草" not in system_prompt, "Player B's private clue content leaked into Player A's public prompt"
    ...
    async def test_public_prompt_includes_own_private_clue(self, mock_llm, collab_session):
        ...
        assert "银行账户" in system_prompt or "南海贸易" in system_prompt, "Player A's own private clue should appear in their public chat prompt"

    async def test_public_dm_audience_instruction_present(self, mock_llm, collab_session):
        ...
        assert "公开对话" in system_prompt, "Public audience instruction missing from public chat prompt"
        assert "绝对不得" in system_prompt, "Leak prevention instruction missing from public chat prompt"

    async def test_public_dm_summary_only_has_titles_not_content(self, mock_llm, collab_session):
        ...
        assert "日记残页" in system_prompt, "DM summary should include Player B's clue title"
        assert "字迹潦草" not in system_prompt, "DM summary must not include Player B's private clue content"

    async def test_no_player_id_falls_back_to_standard_prompt(self, mock_llm, collab_session):
        """dm_turn without player_id uses the standard (non-per-player) prompt."""
        mock_llm.set_response({"judgment": "无关", "response": "继续思考", "truth_progress": 0.0, "should_hint": False})
        await dm_turn(collab_session, "守望者死了吗？")
        system_prompt = mock_llm.last_system_prompt

        # Standard prompt does not have per-player sections
        assert "当前提问玩家的私有线索" not in system_prompt, "Standard prompt should not include per-player sections when player_id is absent"
...
class TestPrivateResponseNotBroadcast:
    ...
    async def test_game_session_consecutive_misses_not_incremented(self, mock_llm, collab_session):
        ...
        assert collab_session.consecutive_misses == 2, "dm_turn_private must not alter session.consecutive_misses"

    async def test_game_session_not_finished_after_private_chat(self, mock_llm, collab_session):
        """Even if the private DM mock returns truth_progress=1.0, session must not finish."""
        mock_llm.set_response({"judgment": "是", "response": "你完全猜对了！", "truth_progress": 1.0, "should_hint": False, "audience": "private"})
        await dm_turn_private(collab_session, "uid-A", "守望者是被良心折磨死的吗？")
        assert not collab_session.finished, "dm_turn_private must not mark session as finished — no game state changes"

    async def test_unlocked_clue_ids_not_modified_by_private_chat(self, mock_llm, collab_session):
        """Private chat must not unlock clue cards."""
        mock_llm.set_response({"judgment": "是", "response": "灯塔是被关掉的", "truth_progress": 0.4, "should_hint": False, "audience": "private"})
        ...
        assert collab_session.unlocked_clue_ids == initial_unlocked, "dm_turn_private must not unlock clue cards"

    async def test_history_length_unchanged_after_multiple_private_chats(self, mock_llm, collab_session):
        ...
        assert len(collab_session.history) == 0, "Three private chats must leave history empty"
